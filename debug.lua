...
function hrp4_proto.dissector(buffer,pinfo,tree)
  ...
  -- TTL
  subtree:add(buffer(15,1),"TTL: " .. buffer(15,1):uint())
  -- Reserved
  subtree:add(buffer(16,3),"Reserved: " .. buffer(16,3):uint())

  -- Switch according to datatype.
  datatype = buffer(20,4):string()

  if datatype == "BCN4" then
    -- pinfo.cols.info = sVersion .. "Beacon Packet"
    local inner = subtree:add(hrp4_proto,buffer(),"Beacon Packet")
    -- Routing Entry
    for i=24,buffer:len(),12 do
        local entry = subtree:add(inner,buffer(),"Routing Entry")
