...
function hrp4_proto.dissector(buffer, pinfo, tree)
    ...
    elseif datatype == "RTP4" then
        ...
        flags:add(hrp4_proto, buffer(32, 1), "SYN: " .. buffer(32, 1):bitfield(0, 1))
        flags:add(hrp4_proto, buffer(32, 1), "ACK: " .. buffer(32, 1):bitfield(1, 1))
        flags:add(hrp4_proto, buffer(32, 1), "FIN: " .. buffer(32, 1):bitfield(2, 1))
        flags:add(hrp4_proto, buffer(32, 1), "RST: " .. buffer(32, 1):bitfield(3, 1))

        inner:add(hrp4_proto, buffer(34, 2), "Window Size: " .. buffer(34, 2):uint())
        inner:add(hrp4_proto, buffer(36), "Payload: " .. buffer(36):string())
    else
